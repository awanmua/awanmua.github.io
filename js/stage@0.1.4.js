...
//突发的变化
//-不再在触摸处理程序中使用preventDefault()。
//-<canvas>元素应用了' touchAction:auto '样式。
//内联的阶段。js依赖性:股票。js
/**
 *Ticker.js
 *requestAnimationFrame帮手。为回调函数提供帧之间的运行时间和延迟补偿乘数。
 *作者:Caleb Miller
 *caleb@caleb-miller.com
 *Stage.js
 *超级简单的画布“舞台”抽象。结合Ticker。Js，它有助于简化:
 *-准备绘画用的画布
 *-高分辨率渲染。
 *-调整画布的大小。
 *-指针事件(鼠标和触摸)。
 *-帧回调与有用的计时数据和计算滞后。
 *这不是健壮的画布绘图库的替代品;它被设计成尽可能轻量和延迟
 *用户的全部渲染负担。
 *作者:Caleb Miller
 *caleb@caleb-miller.com
*/
